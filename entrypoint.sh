...
for i in $(seq 1 30); do
    if ejabberdctl status 2>/dev/null | grep -q "is running"; then
...
if ! ejabberdctl check_account "$ADMIN_USER" "$DOMAIN" 2>/dev/null; then
    echo "[entrypoint] Creando admin: ${ADMIN_USER}@${DOMAIN}"
    ejabberdctl register "$ADMIN_USER" "$DOMAIN" "$ADMIN_PASS"
